...
public class ForceTubeVRInterface : MonoBehaviour
{
    ...
    public static void Shoot(ProTubeSettings settings, ForceTubeVRChannel target = ForceTubeVRChannel.rifle){
        Shoot(settings.GetKickPower(), settings.rumblePower, settings.rumbleDuration, target);
    }

    public static void Kick(ProTubeSettings settings, ForceTubeVRChannel target = ForceTubeVRChannel.rifle){
        Kick(settings.GetKickPower(), target);
    }

    public static void Rumble(ProTubeSettings settings, ForceTubeVRChannel target = ForceTubeVRChannel.rifle){
        Rumble(settings.rumblePower, settings.rumbleDuration, target);
    }
...
public static void SwitchChannel(int fromChannel, int toChannel, string sName)
    {
        RemoveFromChannel(fromChannel, sName);
        AddToChannel(toChannel, sName);
    }
